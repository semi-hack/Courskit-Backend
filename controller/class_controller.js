...
const createclass = async (req, res) => {
    const { name, Courses, AcademicPeriod, Meeting, Population, UnavailableRooms } = req.body;
...
const getClass = async (req, res) => {
    const classes = await Klass.find({}).populate('Courses').populate('AcademicPeriod');
  if (classes) {
...
const UpdateClass = async (req, res) => {
    ...
    const UpdatedClass = await Klass.findByIdAndUpdate(req.headers._id, {$set: req.body}, { new: true });
    if (!UpdatedClass) {
        res.status(400).json({
